...
import {HeximalDeclarativeElement} from './declarative-element.js';
 ...
 */
export class HeximalDefineElement extends HTMLElement {
  static observedAttributes = ['name', 'properties'];

  private declare _initialized: boolean;

  class?: typeof HeximalDeclarativeElement;
  ...
  connectedCallback() {
    if (!this._initialized) {
      ...
      let superclass = HeximalDeclarativeElement;
      ...
      if (extendsName !== null) {
        ...
        const foundSuperclass = (superDefinition as HeximalDefineElement).class;
        if (foundSuperclass) {
          superclass = foundSuperclass;
        }
        superTemplate = (superDefinition as HeximalDefineElement).template;
      }
...
customElements.define('h-define-element', HeximalDefineElement);
