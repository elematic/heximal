...
suite('Parser', function() {
  ...
  test('should parse binary operators', function() {
    ...
    for (const i in operators) {
      ...
      expectParse(
          `this ${op} null`,
          astFactory.binary(
              astFactory.id('this'), op, astFactory.literal(null)));
    }
  });
  ...
  test('should parse a method call with multiple arguments', function() {
    expectParse(
        'a.b(c, d)',
        astFactory.invoke(
            astFactory.id('a'), 'b', [astFactory.id('c'), astFactory.id('d')]));
  });
  ...
  test('should parse chained method calls', function() {
    expectParse(
        'a.b().c()',
        astFactory.invoke(
            astFactory.invoke(astFactory.id('a'), 'b', []), 'c', []));
  });
  ...
  test('should parse chained function calls', function() {
    expectParse(
        'a()()',
        astFactory.invoke(
            astFactory.invoke(astFactory.id('a'), null, []), null, []));
  });
  ...
  test('should parse parenthesized expression', function() {
    ...
    expectParse(
        '(( 3 * ((1 + 2)) ))',
        astFactory.paren(astFactory.paren(astFactory.binary(
            astFactory.literal(3),
            '*',
            astFactory.paren(astFactory.paren(astFactory.binary(
                astFactory.literal(1), '+', astFactory.literal(2))))))));
  });

  test('should parse an index operator', function() {
    expectParse(
        'a[b]', astFactory.index(astFactory.id('a'), astFactory.id('b')));
    expectParse(
        'a.b[c]',
        astFactory.index(
            astFactory.getter(astFactory.id('a'), 'b'), astFactory.id('c')));
  });
  ...
  test('should parse ternary operators', function() {
    expectParse(
        'a ? b : c',
        astFactory.ternary(
            astFactory.id('a'), astFactory.id('b'), astFactory.id('c')));
    expectParse(
        'a.a ? b.a : c.a',
        astFactory.ternary(
  ...
  test('should parse map literals', function() {
    ...
    expectParse('{\'a\': 1, \'b\': 2 + 3}', astFactory.map({
      'a': astFactory.literal(1),
      'b': astFactory.binary(astFactory.literal(2), '+', astFactory.literal(3)),
    }));
    expectParse('{\'a\': foo()}', astFactory.map({
      'a': astFactory.invoke(astFactory.id('foo'), null, []),
    }));
    expectParse('{\'a\': foo(\'a\')}', astFactory.map({
      'a': astFactory.invoke(
          astFactory.id('foo'), null, [astFactory.literal('a')]),
    }));
  });
  ...
  test('should parse map literals with method calls', function() {
    expectParse(
        '{\'a\': 1}.length',
        astFactory.getter(
            astFactory.map({'a': astFactory.literal(1)}), 'length'));
  });

  test('should parse list literals', function() {
    expectParse('[1, "a", b]', astFactory.list([
      astFactory.literal(1),
      astFactory.literal('a'),
      astFactory.id('b')
    ]));
    expectParse('[[1, 2], [3, 4]]', astFactory.list([
